    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Manage hostel blocks, rooms, and student allocations</p>
        </div>
        <Button className="gradient-primary border-0 text-primary-foreground"><Plus className="h-4 w-4 mr-2" />Add Block</Button>
      </div>
      ...
      <SimpleCard>
        <div className="mb-6 relative max-w-sm">
          ...
          <Input placeholder="Search hostels..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9" />
        </div>
                ...
                <div className="h-2 rounded-full bg-muted overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${
                      (hostel.occupied / hostel.capacity) > 0.9 ? "bg-destructive" :
                      (hostel.occupied / hostel.capacity) > 0.7 ? "bg-warning" : "bg-success"
                    }`}
